...
- Older recordings estimate media zero from captured clicks, else from the recording start, and the UI labels their times as approximate.
...
- [x] Older recordings show an approximate label.
...
- Large imports remain responsive: the row list renders only visible rows.
...
- Inferring pauses for older recordings.

## Source Ownership

`SessionClock` and `RecordingController` capture media timing; `RecordingRepository` and `TimelineImportRepository` persist it and imported rows (migration `0003`). `@path/timeline` owns parsing (`TimelineImportParser`) and alignment (`MediaTiming`). `TimelineImportService` owns import validation and prompt evidence. `WorkspacePage` owns the review tab and `useTimelineImports` above the status-keyed `RecordingPane`; `TimelineTabs` and `TimelineImportPanel` render them.
...
- Built renderer checked with a mocked bridge in light and dark themes at 1480×900 and 1180×720: tabs, virtualized rows (400 rows, about 16 rendered), element breadcrumbs, the approximate label, no horizontal overflow, and the Settings limit field.

Unverified: the native file dialog and a live import inside the running Electron app, and migration `0003` on an existing user database (applied at next launch).
